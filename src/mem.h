...
namespace mem {
	...
	namespace ex {
		...
		// Parameters:
		// [in] hProc:		Handle to the target process.
		...
		// [in] detour:		Address of the function that should be executed on a call of the origin function within the virtual address space of the target process.
		//					Has to use the same calling convention as the origin function for uninterrupted process execution.
		// [in] originCall: Address of the call of the origin function call within the detour function. The call should be of the same calling convention as the origin function.
		...
		// Return:			Pointer to the gateway within the virtual address space of the target process.
		...
		// [in] address:	The address within the virtual address space of the target process from which the allocated memory should be reachable by a relative jump (op code: E9).
		// [in] size:		Size of the allocated memory region in bytes.
		// Return:			Pointer to the allocated memory region.
		//					A nullpointer if no memory is available or the function failed for other reasons.
		...
		// [in] origin:	The address that should be patched with the jump instrucion within the virtual address space of the external process. At least five bytes will be overwritten.
		// [in] detour:	The address that should be jumped to within the virtual address space of the target process.
		//				Has to be within reach by a relative jump from the origin address (abs(origin - detour) < UNIT32_MAX).
		// [in] size:	Number of bytes that get overwritten by the jump at the origin address.
		//				Has to be at least five! Only complete instructions should be overwritten!
		...
		// Return:		Pointer to the first bytes after the origin address that have not been patched by the jump or a nullpointer on failure.
		...
		// [in] hProc:	Handle to the target process.
		//				Needs at least PROCESS_VM_OPERATION, PROCESS_VM_READ and PROCESS_VM_WRITE access rights.
		// [in] origin:	The address that should be patched with the jump instrucion within the virtual address space of the external process. At least 13 bytes will be overwritten.
		// [in] detour:	The address that should be jumped to within the virtual address space of the target process.
		// [in] size:	Number of bytes that get overwritten by the jump at the origin address.
		...
		// [in] hProc:	Handle to the target process.
		//				Needs at least PROCESS_VM_OPERATION, PROCESS_VM_READ and PROCESS_VM_WRITE access rights.
		// [in] dst:	The address that should be patched with NOPs within the virtual address space of the target process.
		// [in] size:	Amount of bytes that should be overwritten by a NOP instruction.
		...
		// [in] hProc:	Handle to the target process.
		//				Needs at least PROCESS_VM_OPERATION, PROCESS_VM_READ and PROCESS_VM_WRITE access rights.
		// [in] dst:	The address that should be patched within the virtual address space of the target process.
		// [in] src:	The buffer that should be patched into the external process. Has to be allocated in the virtual memory of the caller process.
		// [in] size:	Size of the source buffer. Beware of buffer overflow.
		// Return:		True on success, false on failure.
		...
		// [in] base:	The address of the base pointer within the virtual address space of the target process. This is typically a static address.
		// [in] src:	Buffer for the offsets.
		// [in] size:	Size of the offset buffer. Beware of buffer overflow.
		// Return:		The address pointed to by dereferencing the multi level pointer or a nullpointer on failure.
		...
		// [in] base:		Address where the search should start.
		// [in] size:		Amount of bytes that should be searched.
		// [in] signature:	The byte signature base hex that should be looked for as a null terminated string.
		//					Bytes have to be two characters and separeted by spaces. ?? can be used as wildcards.
		//					Example: "DE AD ?? EF"
		// Return:			The address where the byte signature was found within the virtual address space of the target process.
		///					A nullpointer if the signature was not found or the function failed.
		...
		// [in] hProc:	Handle to the target process.
		//				Needs at least PROCESS_VM_READ access rights.
		// [out] dst:	Target buffer for the string.
		// [in] src:	Address of the string within the virtual address space of the target buffer.
		// [in] size:	Size of the target buffer. Beware of buffer overflows.
		// Return:		True on success, false on failure or if no null char was copied.
		...
		// [in] hProc:		Handle to the target process.
		//					Needs at least PROCESS_VM_READ access rights.
		// [in] listEntry:	List entry that should be unlinked.
		// Return:			True on success, false on failure.
	...
	namespace in {
		...
		// as the hooked function for uninterrupted process execution.
		// Parameters:
		// [in] origin:		Address of the origin function to be hooked within the virtual address space of the caller process. At least the first five bytes will be overwritten.
		// [in] detour:		Address of the function that should be executed on a call of the origin function within the virtual address space of the caller process.
		//					Has to use the same calling convention as the origin function for uninterrupted process execution.
		// [in] size:		Number of bytes that get overwritten by the jump at the beginning of the origin function.
		//					The overwritten instructions get executed by the gateway right before executing the origin function.
		//					Has to be at least five! Only complete instructions should be overwritten!
		//					It is necessary to look at the disassembly of the origin function to find out when the first complete instruction finishes after the first five bytes.
		// Return:			Pointer to the gateway. This address should be called by the detour function with the same calling convention as the origin function.
		//					The stolen bytes of the orgin function are located here.
		...
		// The Win32 APIs VirtualAllocEx can be used as an alternative for x86.
		// Parameters:
		// [in] address:	The address within the virtual address space of the caller process from which the allocated memory should be reachable by a relative jump (op code: E9).
		// [in] size:		Size of the allocated memory region in bytes.
		// Return:			Pointer to the allocated memory region
		//					A nullpointer if no memory is available or the function failed.
		...
		// Parameters:
		// [in] origin:	The address that should be patched with the jump instrucion within the virtual address space of the caller process. At least five bytes will be overwritten.
		// [in] detour:	The address that should be jumped to within the virtual address space of the caller process.
		//				Has to be within reach by a relative jump from the origin address (abs(origin - detour) < UNIT32_MAX).
		// [in] size:	Number of bytes that get overwritten by the jump at the origin address.
		//				Has to be at least five! Only complete instructions should be overwritten!
		//				It is necessary to look at the disassembly at the origin address to find out when the first complete instruction finishes after the first five bytes.
		...
		// Can only be used in an x64 process.
		// Parameters:
		// [in] origin:	The address that should be patched with the jump instrucion within the virtual address space of the caller process. At least 13 bytes will be overwritten.
		// [in] detour:	The address that should be jumped to within the virtual address space of the target process.
		// [in] size:	Number of bytes that get overwritten by the jump at the origin address.
		//				Has to be at least 13! Only complete instructions should be overwritten!
		//				It is necessary to look at the disassembly at the origin address to find out when the first complete instruction finishes after the first 13 bytes.
		...
		// Parameters:
		// [in] dst:	The address that should be patched with NOPs within the virtual address space of the caller process.
		// [in] size:	Amount of bytes that should be overwritten by a NOP instruction.
		// Return:		True on success, false on failure.
		...
		// Parameters:
		// [in] dst:	The address that should be patched within the virtual address space of the caller process.
		// [in] src:	The buffer that should be patched into the target process. Has to be allocated in the virtual memory of the caller process.
		// [in] size:	Size of the source buffer. Beware of buffer overflow.
		// Return:		True on success, false on failure.
		...
		// Parameters:
		// [in] base:	The address of the base pointer within the virtual address space of the caller process. This is typically a static address.
		// [in] src:	Buffer for the offsets.
		// [in] size:	Size of the offset buffer. Beware of buffer overflow.
		// Return:		The address pointed to by dereferencing the multi level pointer or a nullpointer on failure.
		//				Example: *(*(*(base + offset[0]) + offset[1]) + offset[2])
		...
		//					Bytes have to be two characters and separeted by spaces. ?? can be used as wildcards.
		//					Example: "DE AD ?? EF"
		// Return:			The address where the byte signature was found within the virtual address space of the caller process.
		//					A nullpointer if the signature was not found or the function failed.
	...
	namespace helper {
		...
		// Parameters:
		// [in] charSig:	The byte signature base hex that should be looked for as a null terminated string.
		//					Bytes have to be two characters and separeted by spaces. ?? can be used as wildcards and will be converted to -1.
		//					Example: "DE AD ?? EF"
		// [out] intSig:	Buffer for the integer signature. Enough memory has to be allocted to hold all the bytes from the charSig.
		// [in] size:		Size of the buffer for the integer signature.
		// Return:			True on success, false if the buffer has an invalid size.
		...
		// Parameters:
		// [in] base:		Address where the search should start.
		// [in] size:		Amount of bytes that should be searched. Should not cross memory region boundries.
		// [in] signature:	The byte signature that should be looked for as an array of integers.
		//					Values have to be between -1 and 255. -1 is treated as a wildcard.
		// Return:			The address where the byte signature was found within the memory region of the caller process.
		//					A nullpointer if the signature was not found.
